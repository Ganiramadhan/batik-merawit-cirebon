...
import { CiBarcode, CiUser, CiDroplet , CiCalendarDate } from "react-icons/ci";
...
export default function BatikIndex({ user, batikData, title, members, batikDescription, backLayers, motifCreators}) {
    ...
    const [oldCodeBatik, setOldCodeBatik] = useState("");
    ...
    const generateCodeBatik = async (isEditMode = false) => {

        // console.log('New Batik:', newBatik.id);
        // console.log('Batik Data:', batikData);
        let nomorMotif;
        try {
            if (!isEditMode) {
                // Jika bukan mode edit, generate nomor motif baru
                const response = await axios.get('/batik/get-batik-code');
                const lastMotifCode = response.data.last_motif_code || "000";
                nomorMotif = (parseInt(lastMotifCode, 10) + 1).toString().padStart(3, "0");
            } else {
                // Jika mode edit, ambil nomor motif dari kode batik yang sudah ada
                const existingBatik = filteredData.find(batik => batik.id === newBatik.id);

                nomorMotif = existingBatik.code_batik.slice(1, 4);
            }
        } catch (error) {
            console.error("Gagal mengambil nomor motif terakhir:", error);
            nomorMotif = "000"; // Default jika gagal
        }
        ...
        // Format akhir kode batik
        const generatedCode = `M${nomorMotif}${kualitas}${pemilik}${pembuat}${penembok}${tahun}`;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const generatedCode = await generateCodeBatik(isEditMode);
            const formData = new FormData();
    
            // Tambahkan data yang perlu disubmit
            formData.append("code_batik", generatedCode);
            formData.append("color_materials", setCustomField("color_materials", customColorMaterial, newBatik.color_materials));
            formData.append("name", setCustomField("name", customName, newBatik.name));
            formData.append("materials", newBatik.materials === 'custom' ? customMaterial : newBatik.materials);
            ...
            Object.keys(newBatik).forEach(key => {
                if (!['name', 'materials', 'color_materials', 'image'].includes(key) && newBatik[key]) {
            ...
            if (isEditMode) {
                setFilteredBatikData(prevData =>
                    prevData.map(item => item.id === newBatik.id ? updatedData : item)
                );
            } else {
                setFilteredBatikData(prevData => [...prevData, updatedData]);
            }
            ...
            Swal.fire({
                ...
                text: error.response?.data?.message || 'Gagal mengirimkan data ke server.',
            });
        } finally {
            setIsSubmitting(false);
        }
  ...
  // Convert motifCreators to options for React Select
    ...
    const handleEditClick = (id) => {
        ...
        if (selectedBatik) {
            ...
            setOldCodeBatik(selectedBatik.code_batik || "");
        ...
        <AuthenticatedLayout user={user} header={<h2 className="font-semibold text-xl text-gray-800">{title}</h2>}>
                ...
                {filteredData.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {filteredData.map((batik, index) => (
                            <div
                                key={batik.id || index}
                                className="relative flex flex-col justify-between p-6 bg-white border rounded-lg shadow-md hover:shadow-lg transition-transform duration-300 hover:scale-105 cursor-pointer"
                                onClick={(e) => {
                                    // Abaikan klik pada elemen dengan class no-modal
                                    if (!e.target.closest('.no-modal')) handleCardClick(batik);
                                }}
                            >
                                {/* Quality Badge */}
                                {batik.quality && (
                                ...
                                <div className="flex justify-between items-center gap-2 mt-2">
                                    <button
                                        type="button"
                                        className="bg-blue-500 text-white py-2 px-4 rounded-md shadow-sm flex items-center justify-center hover:bg-blue-600 hover:shadow-md transition-all duration-200 ease-in-out no-modal"
                                        onClick={(e) => {
                                            e.stopPropagation(); // Cegah event bubbling ke parent
                                            handleEditClick(batik.id);
                                        }}
                                    ...
                                    <button
                                        className="bg-red-500 text-white py-2 px-4 rounded-md shadow-sm flex items-center justify-center hover:bg-red-600 hover:shadow-md transition-all duration-200 ease-in-out no-modal"
                                        onClick={(e) => {
                                            e.stopPropagation(); // Cegah event bubbling ke parent
                                            handleDeleteClick(batik.id);
                                        }}
                                    ...
                                    <button
                                        onClick={async (e) => {
                                            ...
                                            const logoSize = 45; // Ukuran logo BTMC dan IGI
                                            const qrSize = 90; // Ukuran QR Code lebih kecil
                                            ...
                                            try {
                                                ...
                                                // Tentukan ukuran gambar member berdasarkan rasio tinggi/lebarnya
                                                let memberImageWidth = logoSize;
                                                let memberImageHeight =
                                                    (memberImage.height / memberImage.width) * memberImageWidth;

                                                if (memberImage.height > memberImage.width) {
                                                    memberImageWidth = logoSize * 0.8;
                                                    memberImageHeight =
                                                        (memberImage.height / memberImage.width) * memberImageWidth;
                                                } else if (memberImage.width > memberImage.height) {
                                                    memberImageWidth = logoSize * 1.5;
                                                    memberImageHeight =
                                                        (memberImage.height / memberImage.width) * memberImageWidth;
                                                }
                                                ...
                                                // Gambar logo BTMC dan IGI
                                                ctx.drawImage(btmcImage, 55, 20, logoSize, logoSize);
                                                ctx.drawImage(igiImage, 105, 20, logoSize, logoSize);

                                                // Posisi gambar member
                                                let memberYPosition = 20;
                                                if (memberImageWidth > logoSize * 1.2) {
                                                    memberYPosition = 30;
                                                }
                                                ...
                                                // Tambahkan Kode Produk
                                                const certificationText =
                                                    "Kode Produk IG Batik Tulis Merawit";
                                                let codeText = batik.code_batik || "-";
                                                codeText = codeText.replace(/[^a-zA-Z0-9]/g, "");
                                                const formattedCode = [
                                                    codeText.charAt(0),
                                                    codeText.slice(1, 4),
                                                    ...codeText.slice(4).match(/.{1,2}/g),
                                                ];
                                                ...
                                                // Gambar kode dalam kotak
                                                const boxSize = 20;
                                                const totalWidth =
                                                    formattedCode.length * boxSize +
                                                    (formattedCode.length - 1) * boxSpacing;
            ...
            {selectedBatik && (
                <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 z-50">
                    <div className="bg-white rounded-lg shadow-lg w-11/12 max-w-2xl p-6 relative overflow-y-auto max-h-screen">
                        {/* Close Button */}
                        {/* <button
                       ...
                       {/* Modal Header */}
                        <div className="text-center mb-6">
                            <h2 className="text-2xl font-extrabold text-gray-700 mb-2 tracking-wide">
                                {selectedBatik.name || "-"}
                            </h2>
                            <p className="text-md text-gray-600 italic">
                                Informasi Detail Batik
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {/* Image Section */}
                            <div className="flex-shrink-0 md:col-span-2 flex justify-center">
                                <img
                                    src={`/storage/${selectedBatik.image}`}
                                    alt={selectedBatik.name}
                                    className="w-full max-w-xs h-48 object-cover rounded-lg shadow-lg"
                                />
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-gray-700">
                                    <strong className="font-bold">Anggota MPIG-BTMC:</strong> {selectedBatik.member?.name || "-"}
                                </span>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-gray-700">
                                    <strong className="font-bold">Nama Toko:</strong> {selectedBatik.member?.store_name || "-"}
                                </span>
                            </div>

                            {/* Pembuat Motif */}
                            <div className="flex items-center gap-2">
                                <PiPaintBrushLight className="text-blue-500 text-lg" />
                                <span className="text-sm text-gray-700">
                                    <strong className="font-bold">Pembuat Motif:</strong> {selectedBatik.motif_creator || "-"}
                                </span>
                            </div>

                            {/* Tukang Penembok */}
                            <div className="flex items-center gap-2">
                                <TfiPaintBucket className="text-blue-500 text-lg" />
                                <span className="text-sm text-gray-700">
                                    <strong className="font-bold">Penembok:</strong> {selectedBatik.bricklayer_name || "-"}
                                </span>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-gray-700">
                                    <strong className="font-bold">Bahan Kain:</strong> {selectedBatik.materials || "-"}
                                </span>
                            </div>

                            {/* Bahan Pewarna */}
                            <div className="flex items-center gap-2">
                                <CiDroplet  className="text-blue-500 text-lg" />
                                <span className="text-sm text-gray-700">
                                    <strong className="font-bold">Bahan Pewarna:</strong> {selectedBatik.color_materials || "-"}
                                </span>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-gray-700">
                                    <strong className="font-bold">Kode Batik: </strong> 
                                    {`M-${selectedBatik.code_batik.slice(1, 4)}-${selectedBatik.code_batik.slice(4).match(/.{1,2}/g)?.join("-") || ""}`}
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-gray-700">
                                    <strong className="font-bold">Tahun Produksi:</strong> {selectedBatik.production_year || "-"}
                                </span>
                            </div>
                            ...
                            <div className="md:col-span-2 flex items-start gap-2">
                                <p className="text-sm text-gray-700 text-justify">{selectedBatik.description || "-"}</p>
                            </div>
                        </div>

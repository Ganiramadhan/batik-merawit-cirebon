...
export default function MotifCreatorData({ user, title, motifCreators }) {
    ...
    const [newMotifCreator, setNewMotifCreator] = useState({
        name: '',
    });
    
    
    const [filteredMotifCreatorData, setFilteredMotifCreatorData] = useState(motifCreators);

    
    const filteredData = filteredMotifCreatorData.filter((motifCreators) => {
        const searchQueryLower = searchQuery.toLowerCase();
        return Object.values(motifCreators)
    ...
    const handleSubmit = async (e) => {
        ...
        for (const key in newMotifCreator) {
            formData.append(key, newMotifCreator[key]);
        }
        ...
        try {
            ...
            // Edit mode (update)
            if (isEditMode) {
                response = await axios.post(`/motif_creator/${newMotifCreator.id}`, formData);

                if (response.status === 200 || response.status === 204) {
                    const updatedImage = response?.data?.member?.image;
                    setFilteredMotifCreatorData((prevData) => {
                        return prevData.map((item) =>
                            item.id === newMotifCreator.id
                                ? {
                                    ...item,
                                    ...newMotifCreator,
                                    name: response.data.motif_creator.name || '',
                                }
                                : item
                        );
                    });
                ...
                // Add new member
                response = await axios.post('/motif_creator', formData);
                setFilteredMotifCreatorData((prevData) => [
                    ...prevData,
                    {
                        ...response.data.motif_creator,
                        name: response.data.motif_creator.name || '',
                    },
                ]);
    ...
    const handleEditClick = (id) => {
        const motifCreator = filteredMotifCreatorData.find((motifCreator) => motifCreator.id === id);
    
        if (motifCreator) {
            setNewMotifCreator({
                ...motifCreator,  
                name: motifCreator.name || '', 
            });
            setIsEditMode(true);  
            setIsModalOpen(true); 
        }
    ...
    const handleDeleteClick = async (id) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await axios.delete(`/motif_creator/delete/${id}`, {
                            headers: {
                        ...
                        if (response.data.success) {
                            setFilteredMotifCreatorData((prevData) =>
                                prevData.filter((motif_creator) => motif_creator.id !== id)
                            );
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Terjadi Kesalahan',
                            text: 'Data Pembuat Motif tidak dapat dihapus.',
                        });
                    }
                }
            });
            ...
            {/* Search and Add backlayer Section */}
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8 bg-white p-6 rounded-lg shadow-md">
                    ...
                    <div className="relative w-full sm:w-1/3">
                        <input
                            type="text"
                            placeholder="Cari pembuat motif..."
                            value={searchQuery}
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            onClick={() => {
                                setIsEditMode(false);
                                setNewMotifCreator({ backlayer_number: '', name: ''});
                                setIsModalOpen(true);
                            }}
                    ...
                    <table className="table-auto border-collapse w-full text-sm border border-gray-300 rounded-lg shadow-md">
                        <thead className="bg-gradient-to-r from-blue-500 to-blue-600 text-white">
                            ...
                            {currentData.length > 0 ? (
                                currentData.map((motif_creator, index) => (
                                    <tr
                                        key={motif_creator.id}
                                        className={`border-b ${
                                            index % 2 === 0 ? 'bg-white' : 'bg-gray-50'
                                        } hover:bg-blue-100 transition-all`}
                                    >
                                        <td className="text-center text-gray-700 border border-gray-300 py-3">
                                            {motif_creator.motif_creator_number}
                                        </td>
                                        ...
                                        <td className="px-4 py-3 text-center border border-gray-300">
                                            <div className="flex justify-center gap-4">
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => handleEditClick(motif_creator.id)}
                                                >
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => handleDeleteClick(motif_creator.id)}
                                                >
                        ...
                        <div className="flex items-center justify-between border-b pb-3 mb-4">
                            <div className="flex items-center space-x-2">
                                ...
                                <h3 className="text-xl font-semibold text-gray-800">
                                    {isEditMode ? 'Edit Data Pembuat Motif' : 'Tambah Data Pembuat Motif'}
                                </h3>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="col-span-2">
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Nama Pembuat Motif</label>
                                    <input
                                        type="text"
                                        value={newMotifCreator.name || ''}
                                        onChange={(e) => setNewMotifCreator({ ...newMotifCreator, name: e.target.value })}
